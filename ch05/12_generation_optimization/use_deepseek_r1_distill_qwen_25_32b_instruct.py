...
dotenv.load_dotenv(".siliconflow.v2.env")
...
llm = ChatOpenAI(model="deepseek-ai/DeepSeek-R1-Distill-Qwen-32B")
...
def convert_markdown_json_to_dict(markdown_json: str) -> dict:
    # Remove the markdown syntax and load the JSON string into a dictionary
    json_string = markdown_json.strip('```json\n').strip('```').strip()
    return json.loads(json_string)

with open("../03_eval/data_eval/v20241219/qa_pairs_rewrite.json", "r") as f:
    qa_pairs = json.load(f)
    for doc in tqdm(qa_pairs[:101]):
        query = doc["query"]
        try:
            result = rag_chain.invoke(query)
            rsp_dict = convert_markdown_json_to_dict(result["response"].content)
            doc["response"] = {
                "content": rsp_dict["answer"],
                "contexts": [result["context"],],
                "selected_content": rsp_dict["selected_content"],
            }
            results.append(doc)
...
output_path = "../03_eval/data_metrics/v20241219/ch0507_deepseek_r1_distill_qwen25_32b_instruct/response.json"
